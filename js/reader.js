...
*/
  ...
  var Enumerator = (function () {
    ...
    Enumerator.prototype._settleMaybeThenable = function _settleMaybeThenable(
      entry,
      i,
    ) {
...
*/
  function all(entries, label) {
    ...
    return new Enumerator(this, entries, true /* abort on reject */, label)
      .promise;
  }
  ...
  var PromiseHash = (function (_Enumerator) {
    ...
    function PromiseHash(Constructor, object) {
      ...
      return _possibleConstructorReturn$1(
        this,
        _Enumerator.call(this, Constructor, object, abortOnReject, label),
      );
    }
  ...
  var HashSettled = (function (_PromiseHash) {
    ...
    function HashSettled(Constructor, object, label) {
      return _possibleConstructorReturn$2(
        this,
        _PromiseHash.call(this, Constructor, object, false, label),
      );
    }
...
*/
  ...
  function hashSettled(object, label) {
    if (!isObject(object)) {
      return Promise.reject(
        new TypeError("RSVP.hashSettled must be called with an object"),
        label,
      );
    }
...
*/
  function map(promises, mapFn, label) {
    ...
    if (!isFunction(mapFn)) {
      return Promise.reject(
        new TypeError("RSVP.map expects a function as a second argument"),
        label,
      );
    }
...
*/
  ...
  function filter(promises, filterFn, label) {
    if (
      !isArray(promises) &&
      !(isObject(promises) && promises.then !== undefined)
    ) {
      return Promise.reject(
        new TypeError("RSVP.filter must be called with an array or promise"),
        label,
      );
    }
    ...
    if (!isFunction(filterFn)) {
      return Promise.reject(
        new TypeError("RSVP.filter expects function as a second argument"),
        label,
      );
    }

    var promise = isArray(promises)
      ? resolveAll(promises, label)
      : resolveSingle(promises, label);
  ...
  var rsvp =
    ...
    _defineProperty(_asap$cast$Promise$Ev, "async", async),
    _defineProperty(_asap$cast$Promise$Ev, "filter", filter),
    _asap$cast$Promise$Ev);
...
EPUBJS.core.prefixed = function (unprefixed) {
  ...
  for (var i = 0; i < length; i++) {
    if (
      typeof document.documentElement.style[vendors[i] + upper] != "undefined"
    ) {
...
EPUBJS.core.indexOfSorted = function (
  item,
  array,
  compareFunction,
  _start,
  ...
  if (compared === -1) {
    ...
    return EPUBJS.core.indexOfSorted(
      item,
      array,
      compareFunction,
      start,
...
EPUBJS.core.getElementTreeXPath = function (element) {
  var paths = [];
  var isXhtml =
    element.ownerDocument.documentElement.getAttribute("xmlns") ===
...
EPUBJS.Reader = function (bookPath, _options) {
  ...
  book.ready
    .then(
      function () {
        reader.ReaderController = EPUBJS.reader.ReaderController.call(
          reader,
          book,
        );
        reader.SettingsController = EPUBJS.reader.SettingsController.call(
          reader,
          book,
        );
        reader.ControlsController = EPUBJS.reader.ControlsController.call(
          reader,
          book,
        );
        reader.SidebarController = EPUBJS.reader.SidebarController.call(
          reader,
          book,
        );
        reader.BookmarksController = EPUBJS.reader.BookmarksController.call(
          reader,
          book,
        );
        reader.NotesController = EPUBJS.reader.NotesController.call(
          reader,
          book,
        );
        ...
        this.rendition.on(
          "keydown",
          reader.ReaderController.arrowKeys.bind(this),
        );
...
EPUBJS.Reader.prototype.saveSettings = function () {
  if (this.book) {
    this.settings.previousLocationCfi =
      this.rendition.currentLocation().start.cfi;
  }
...
EPUBJS.reader.ControlsController = function (book) {
  ...
  rendition.on("relocated", function (location) {
    ...
    // Update the History Location
    if (reader.settings.history && window.location.hash != cfiFragment) {
      // Add CFI fragment to the history
      history.pushState({}, "", cfiFragment);
    }
  });
...
EPUBJS.reader.NotesController = function () {
  ...
  var insertAtPoint = function (e) {
    ...
    cfi = epubcfi.generateCfiFromTextNode(
      textNode,
      offset,
...
EPUBJS.reader.SidebarController = function (book) {
  ...
  var changePanelTo = function (viewName) {
    ...
    if (
      ...
      typeof reader[controllerName] === "undefined"
    )
      return;
